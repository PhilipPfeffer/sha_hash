...
// @spatial object BTC_WEBSITE extends SpatialApp {
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
    ...
    Accel{
      ...
      def sha_transform_m(outer_i: I32): Unit = {
        val A = Reg[ULong]
        val B = Reg[ULong]
        val C = Reg[ULong]
        val D = Reg[ULong]
        ...
        Foreach(64 by 1){ i =>
          val tmp1 = H + EP1(E) + CH(E,F,G) + K_LUT(i) + emm(outer_i, i)
          val tmp2 = EP0(A) + MAJ(A,B,C)
          // println(" " + i + " : " + A.value + " " + B.value + " " +
          //   C.value + " " + D.value + " " + E.value + " " + F.value + " " + G.value + " " + H.value)
          // println("    " + H.value + " " + EP1(E) + " " + CH(E,F,G) + " " + K_LUT(i) + " " + m(i))
      ...
      def SHA256(): Unit = {
        // Initialise h0-h7 
        // QUESTION: why does this happen twice?
        // Pipe{
        //   bitlen(0) = 0.to[ULong]
        //   bitlen(1) = 0.to[ULong]
        //   state(0) = 0x6a09e667L.to[ULong]
        ...
        Sequential.Foreach(0 until len.value by 64 par 1) { i =>
          ...
          if (datalen.value == 64.to[Int]) {
            ...
            // Creation of m array
            // QUESTION: If there's no Sequential. , is it automatically parallelised?
            // assmued to be pipelined
            Foreach(0 until 64 by 1){ii =>
              if ( ii < 16 ) {
                val j = ii << 2 // PHIL
                println(" emm(" + counter + ", " + ii + ") = " + {(data(j).as[ULong] << 24) | (data(j+1).as[ULong] << 16) | (data(j+2).as[ULong] << 8) | (data(j+3).as[ULong])})
                emm(counter, ii) = (data(j).as[ULong] << 24) | (data(j+1).as[ULong] << 16) | (data(j+2).as[ULong] << 8) | (data(j+3).as[ULong])
              } else {
                println(" emm(" + counter + ", " + ii + ") = " + SIG1(emm(counter, ii-2)) + " " + emm(counter, ii-7) + " " + SIG0(emm(counter, ii-15)) + " " + emm(counter, ii-16))
                emm(counter, ii) = SIG1(emm(counter, ii-2)) + emm(counter, ii-7) + SIG0(emm(counter, ii-15)) + emm(counter, ii-16)
              }
              // val j = 4*i
              // m(i) = if (i < 16) {(data(j).as[ULong] << 24) | (data(j+1).as[ULong] << 16) | (data(j+2).as[ULong] << 8) | (data(j+3).as[ULong])}
              //        else {SIG1(m(i-2)) + m(i-7) + SIG0(m(i-15)) + m(i-16)}
            ...
            // 
            // sha_transform_m(counter)
            // DBL_INT_ADD(512);
          }
          counter :+= 1
        }

        val counter_transform = Reg[Int](0)
        Sequential.Foreach(0 until len.value by 64 par 1) { i =>
          datalen := min(len.value - i, 64)
          if (datalen.value == 64.to[Int]) {
            sha_transform_m(counter_transform)
            DBL_INT_ADD(512);
          }
          counter_transform :+= 1
        }
        ...
        if (datalen.value >= 56) {
          ...
          Foreach(0 until 64 by 1){i =>
            if ( i < 16 ) {
              val j = i << 2 // PHIL
              println(" emm(" + counter + ", " + i + ") = " + {(data(j).as[ULong] << 24) | (data(j+1).as[ULong] << 16) | (data(j+2).as[ULong] << 8) | (data(j+3).as[ULong])})
              // println(" m(" + i + ") = " + {(data(j).as[ULong] << 24) | (data(j+1).as[ULong] << 16) | (data(j+2).as[ULong] << 8) | (data(j+3).as[ULong])})
              emm(counter, i) = (data(j).as[ULong] << 24) | (data(j+1).as[ULong] << 16) | (data(j+2).as[ULong] << 8) | (data(j+3).as[ULong])
            } else {
              println(" emm(" + counter + ", " + i + ") = " + SIG1(emm(counter, i-2)) + " " + emm(counter, i-7) + " " + SIG0(emm(counter, i-15)) + " " + emm(counter, i-16))
              // println(" m(" + i + ") = " + SIG1(m(i-2)) + " " + m(i-7) + " " + SIG0(m(i-15)) + " " + m(i-16))
              emm(counter, i) = SIG1(emm(counter, i-2)) + emm(counter, i-7) + SIG0(emm(counter, i-15)) + emm(counter, i-16)
            }
          }
          // 
          sha_transform_m(counter)
          // Foreach(56 by 1){i => data(i) = 0}    // QUESTION: is this necessary since we overwrite? Also, is this parallel?
        }

        DBL_INT_ADD(datalen.value.to[ULong] * 8.to[ULong])
        // TODO: why out of order?
        Pipe{data(63) = (bitlen(0)).to[UInt8]}
        Pipe{data(62) = (bitlen(0) >> 8).to[UInt8]}
        Pipe{data(61) = (bitlen(0) >> 16).to[UInt8]}
        Pipe{data(60) = (bitlen(0) >> 24).to[UInt8]}
      ...
      // Do this once since we're just doing sha hash, not bitcoin proof of work
      // Sequential.Foreach(2 by 1){i =>
      //   // Actually called here
      //   // QUESTION: Why does it happen twice? Seems like second doesn't do anything
      //   // Seems like the reference implementation is wrong.
      //   Pipe{SHA256()}
      //   if (i == 0) {
      //     text_dram(0::32) store hash
      //     len := 32
